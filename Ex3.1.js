...
function nb_year(p0,percent,aug,p){
    let numOfYears=0;
    let isGreater = false;
    let teargetPopulation = p0;
    if(percent!==null) percent = percent/100;
    while(!isGreater){
        if(percent!==null){
        teargetPopulation = teargetPopulation+(teargetPopulation*percent +aug);
        numOfYears+=1;
        }
        else{
            teargetPopulation = teargetPopulation +aug;
            numOfYears+=1;
        }

        if(teargetPopulation>=p){
            isGreater=true;
        }
    }
    return numOfYears;
